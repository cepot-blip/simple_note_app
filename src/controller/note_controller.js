...
import ModelNote from "../model/ModelNote"
env.config()

const note_controller = express.Router()
...
note_controller.post(
	...
	async (req, res) => {
		try {
			...
			const store = await ModelNote.create({
				data: {
					title: data.title,
					body: data.body,
...
//      READ ALL NOTE
note_controller.post("/note/read", async (req, res) => {
	try {
		const { page = 1, limit = 10 } = await req.query
		const { filter } = await req.body
		const skip = (page - 1) * limit
		const result = await ModelNote.findMany({
			skip: parseInt(skip),
			take: parseInt(limit),
			orderBy: {
				id: "desc",
		...
		const cn = await ModelNote.count({
			where: filter,
		})
...
note_controller.put(
	...
	async (req, res) => {
		try {
			const data = await req.body
			const file = await req.file
			const findNote = await ModelNote.findUnique({
				where: {
					id: parseInt(data.id),
				},
				include: {
			...
			if (file) {
				...
				let updateNote = await ModelNote.update({
					where: {
						id: parseInt(data.id),
					},
					data: {
				...
				const updateNote = await ModelNote.update({
					where: {
						id: parseInt(data.id),
					},
					data: {
...
note_controller.delete(
	...
	async (req, res) => {
		try {
			...
			const result = await ModelNote.delete({
				where: {
					id: parseInt(id),
				},
				include: {
